module BoardsWatchers::MessagesActions
  def new_with_watchers
    # Lines added by boards watchers plugin - start
    if !params[:message].nil? && (params[:message]['watcher_user_ids'].blank? && params[:message]['sticky_priority'].blank?)
      return new_without_watchers
    end
    ...
    # Lines added by boards watchers plugin - start
    if params[:message] && !params[:message]['watcher_user_ids'].blank? && User.current.allowed_to?(
      { controller: 'boards_watchers', action: 'manage_topic' }, @project
    )
      @message.watcher_user_ids = params[:message]['watcher_user_ids']
    end
  ...
  def reply_with_watchers
    if @topic && params[:bw_watcher_ids] && User.current.allowed_to?(
      { controller: 'boards_watchers', action: 'manage_topic' }, @project
    )
